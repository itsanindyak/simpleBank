...
func TestTransferTx(t *testing.T) {
	...
	transfers := make(chan TransferTxResult, n)
	...
	for i := 0; i < n; i++ {
		...
		require.NoError(t, err)
		require.Equal(t, fromEntry, result.FromEntry)
	...
	upadatedAccount1, err := store.GetAccount(context.Background(), account1.ID)
	require.NoError(t, err)

	upadatedAccount2, err := store.GetAccount(context.Background(), account2.ID)
	require.NoError(t, err)

	fmt.Println("After transfer >>", upadatedAccount1.Balance, ">>", upadatedAccount2.Balance)
